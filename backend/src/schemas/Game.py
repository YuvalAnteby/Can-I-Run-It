...
class Game(BaseModel):
    ...
    desc: str = Field(
        ...,
        min_length=1,
        description="Description of the game, usually from a game store",
    )
    ...
    available_resolutions: List[str] = Field(
        default_factory=list,
        description="List of available resolutions (e.g., 1080p, 4K)",
    )
    supported_settings: List[str] = Field(
        default_factory=list,
        description="List of supported settings from the lowest to highest (e.g., Low, Medium, High, Ultra)",
    )
    is_ssd_recommended: bool = Field(
        default=False,
        description="True if SSD is recommended for the game by devs or community",
    )
    upscale_support: List[str] = Field(
        default_factory=list,
        description="List of upscaling technologies supported (e.g., DLSS, FSR)",
    )
    api_support: List[str] = Field(
        default_factory=list,
        description="List of graphics APIs supported (e.g., DirectX 11, DirectX 12, Vulkan)",
    )
    created_at: datetime
    id: str
